...
// movement
let velocityx = -3;  // smoother pipe speed
let velocityy = 0;
let gravity = 0.35;  // smoother gravity

// score
let score = 0;

let gameover = false;
crashSound = new Audio('./assets/jump_sound.mp3'); 
jumpSound = new Audio('./assets/jump_sound1.mp3'); 
tickSound = new Audio('./assets/tick_sound.mp3'); 
...
window.onload = function () {
  ...
  setInterval(placepipe, 1800); // smoother pacing
...
function update() {
  ...
  bird.y = Math.min(bird.y, boardheight - bird.height); // adjusted boundary
...
function placepipe() {
  ...
  let openspace = (boardheight / 4) + 40; // smoother spacing
